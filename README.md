...
- **Auto-layout** — select two or more shapes joined by connectors and pick **Lay out vertically** or **Lay out horizontally** (right-click, `⌘K`, or the floating toolbar's **Arrange** menu). The selection is redrawn as a clean layered flow, top to bottom or left to right; the connectors between the shapes are re-routed onto the sides that face each other, and the whole flow lands where the selection already was, so the drawing does not jump. It has no keystroke, and one `⌘Z` takes it back. The placement is the Eclipse Layout Kernel's ([elkjs](https://github.com/kieler/elkjs)), loaded the first time you use it
- **Paste as** — right-click the canvas and turn text on your clipboard into objects. **Paste as sticky notes** makes one note per line, list markers (`-`, `*`, `1.`, `[ ]`) stripped, laid out in a block where you clicked. **Paste Mermaid (flowchart or sequence)** reads [Mermaid](https://mermaid.js.org/syntax/flowchart.html) flowchart source — `graph` / `flowchart` with `TD`, `TB`, `LR`, `RL` or `BT`, the usual node forms (`A[box]`, `A(pill)`, `A{decision}`, `A((circle))`, `A[(store)]`, `A{{hex}}`), links `-->`, `---`, `-.->`, `==>` with labels written either `-->|text|` or `-- text -->`, chains and `&` fan-out — and draws it as real shapes and connectors, auto-laid-out in the direction the chart declares. Either one is a single `⌘Z`. Plain `⌘V` builds the flowchart too when the clipboard holds Mermaid source and nothing of FlowSketch's own has been copied
- **Sequence diagrams** — the same menu item reads [Mermaid `sequenceDiagram`](https://mermaid.js.org/syntax/sequenceDiagram.html) source and draws it: participants in a row across the top, a dashed lifeline down from each, and the messages between them in order down the page. `participant X as Label` and `actor X` are read, and so is anyone a message names without being declared; the arrows `->>`, `-->>`, `->`, `-->`, `-x`, `--x`, `-)` and `--)` become a solid or open arrowhead on a solid or dashed line, and `A->>A` becomes a loop back onto the one lifeline. Activation bars, `Note`s and the `loop` / `alt` / `opt` / `par` blocks are skipped, and the messages inside a block are drawn as ordinary ones. What comes out is ordinary shapes and connectors — restyle them, drag them, export them like anything else — and dragging a participant takes its lifeline and every message on it along
...
- **Share link** — turn on "Anyone with the link can view" from the **Share** button and hand out a `/s/<token>` URL. It opens the board for anyone, signed in or not, with no way to change it. Turning the link off kills that URL for good; turning sharing back on mints a new one. The drawing is the board as it was handed over — but the timer and the round of voting keep up on their own, so a countdown started after somebody opened the link still turns up on their screen.
...
- **Dot voting** — the **Vote** button opens a round: everyone gets the same handful of dots (three by default) to spend across the board, and a `+` appears on every shape. While the round is open you see only *your own* dots, so nobody's choices steer anybody else's; **End voting** closes it and the totals appear on every screen at once. **Sort by votes** lays the chosen shapes out in one row, most votes first, and **Clear votes** wipes the slate for the next round. Revealed counts stay in an exported PNG or SVG — they are part of what the board says — while the voting buttons do not.
- **Board timer** — the **Timer** button starts a shared countdown (1, 3, 5 or 10 minutes, or any length you type) that everybody on the board sees ticking down. It is stored as an *end time*, not a number of seconds, so every window counts down on its own clock and nothing is being written to the diagram while it runs. When it reaches zero the control flashes, reads "Time's up" and **chimes** — a short two-note tone, muted per browser with the speaker in the timer panel — until somebody stops it, and anybody on the board can stop it, whoever started it. A share-link visitor sees the countdown too, keeps up with one started after they opened the page, and cannot start one.
...
| `⌘X` | Cut the selection |
| `⌘V` | Paste the selection — or a Mermaid flowchart or sequence diagram, or a Markdown / CSV / TSV table, from the system clipboard |
