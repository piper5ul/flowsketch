...
- **Paste as** — right-click the canvas and turn text on your clipboard into objects. **Paste as sticky notes** makes one note per line, list markers (`-`, `*`, `1.`, `[ ]`) stripped, laid out in a block where you clicked. **Paste Mermaid as flowchart** reads [Mermaid](https://mermaid.js.org/syntax/flowchart.html) flowchart source — `graph` / `flowchart` with `TD`, `TB`, `LR`, `RL` or `BT`, the usual node forms (`A[box]`, `A(pill)`, `A{decision}`, `A((circle))`, `A[(store)]`, `A{{hex}}`), links `-->`, `---`, `-.->`, `==>` with labels written either `-->|text|` or `-- text -->`, chains and `&` fan-out — and draws it as real shapes and connectors, auto-laid-out in the direction the chart declares. Either one is a single `⌘Z`. Plain `⌘V` builds the flowchart too when the clipboard holds Mermaid source and nothing of FlowSketch's own has been copied
